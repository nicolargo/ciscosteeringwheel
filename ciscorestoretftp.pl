...
# > copy tftp:/@TFTP/CONF system:/running-config 
#
# Syntaxe: ./ciscorestoretftp.pl -a <Cisco IP address/name> -u <login> -p <password> -P <enablepassword> -t <TFTP server IP address> -c <configuration name>
#
...
my $program_name = "ciscorestoretftp.pl";
...
my $confname;			# Name of the new configuration file stored on the TFTP server
my @output;
...
if ($session->enable($enablepassword) ) {
    @output = $session->cmd("copy tftp://$tftp/$confname "
